...
import { type SeoRetainerItem } from './index';
...
interface SeoRetainerEditProps {
    retainer: SeoRetainerItem;
    clients: SimpleClient[];
}

export default function SeoRetainerEdit({ retainer, clients }: SeoRetainerEditProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: `Edit ${retainer.package_name}`,
    ...
    const form = useForm({
        ...
        currency: retainer.currency || 'AED',
        start_date: formatForInput(retainer.start_date),
        billing_day: retainer.billing_day || 1,
    ...
    const handleSubmit: FormEventHandler = (e) => {
        e.preventDefault();
        form.put(`/seo-retainers/${retainer.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit SEO Retainer - ${retainer.package_name}`} />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                            Edit SEO Retainer Package
                        </h1>
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                            Update monthly retainer fee, status, billing schedule, and client assignment.
                        </p>
                    </div>
                        ...
                        >
                            {form.processing ? (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>Updating Retainer...</span>
                                </div>
                            ) : (
                                <span>Update SEO Retainer</span>
                            )}
                        </Button>
                    </div>
                </form>
