...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Client Services', href: '/services' },
...
interface ServiceCategoriesIndexProps {
    categories: PaginatedData<ServiceCategoryItem>;
    stats: {
        total: number;
        active: number;
        inactive: number;
...
export default function ServiceCategoriesIndex({ categories, stats, filters }: ServiceCategoriesIndexProps) {
    ...
    const [deletingCategory, setDeletingCategory] = useState<ServiceCategoryItem | null>(null);
    ...
    const handleCreateSubmit = (e: FormEvent) => {
        e.preventDefault();
        createForm.post('/service-categories', {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleEditOpen = (cat: ServiceCategoryItem) => {
    ...
    const handleEditSubmit = (e: FormEvent) => {
        ...
        editForm.put(`/service-categories/${editingCategory.id}`, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Service Categories CRUD" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                            Define service categories (SEO, PPC, Social Media, Content, Maintenance, etc.) for retainers and client services.
                        </p>
                    </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs">
                    <div className="relative flex-1 w-full md:max-w-md">
                        ...
                        <input
                            ...
                            placeholder="Search service category name..."
                        ...
                        <table className="w-full text-left text-xs text-slate-600 dark:text-slate-300">
                            <thead className="bg-slate-50 dark:bg-slate-950/70 border-b border-slate-200/80 dark:border-slate-800 uppercase tracking-wider text-[11px] font-bold text-slate-500 dark:text-slate-400">
                                <tr>
                                    ...
                                    <th className="px-6 py-4">Assigned Retainers</th>
                                    ...
                                    categories.data.map((cat) => (
                                        <tr key={cat.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                                            <td className="px-6 py-4">
                                                <span className="px-3 py-1.5 rounded-xl text-xs font-extrabold bg-indigo-50 text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                                                    {cat.name}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    {hasPermission(user, 'edit-service-categories') && (
                                                        <button
                                                    ...
                                                    {hasPermission(user, 'delete-service-categories') && (
                                                        <button
                                ...
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400">
                                            Add new service classification type.
                                        </p>
                                    </div>
                                </div>
                                <button
                        ...
                        <div className="w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
                            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        <h3 className="text-base font-extrabold text-slate-900 dark:text-white">
                                            Edit Service Category {editingCategory.name}
                                        </h3>
                                        <p className="text-xs text-slate-400">
