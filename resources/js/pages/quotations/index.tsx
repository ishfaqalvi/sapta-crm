...
export default function QuotationsIndex({
    ...
    const canCreate = hasPermission(user, 'create-quotations');
    const canEdit = hasPermission(user, 'edit-quotations');
    const canDelete = hasPermission(user, 'delete-quotations');
    const canPrint = hasPermission(user, 'print-quotations');
    ...
    const handleDelete = () => {
        ...
        setIsDeleting(true);
        router.delete(`/quotations/${deletingQuotation.id}`, {
            onSuccess: () => {
                setIsDeleting(false);
    ...
    const formatCurrency = (val: number | string, currencyCode: string = 'AED') => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Quotations Directory - Admin Portal" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">
                            Manage and track proposals, formal pricing estimates, and invoices for walk-in and prospective customers.
                        </p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 sm:p-6 max-w-2xl w-full max-h-[90vh] my-auto overflow-y-auto border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4 custom-scrollbar">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 text-xs space-y-1">
                                ...
                                <div className="font-bold text-slate-900 dark:text-white">
                                    {viewingQuotation.customer_prefix ? `${viewingQuotation.customer_prefix} ` : ''}{viewingQuotation.customer_name || 'General Customer'}
                                </div>
                                ...
                                {viewingQuotation.customer_address && (
                                    <div className="text-slate-500 text-[11px]">Address: {viewingQuotation.customer_address}</div>
                                )}
                            </div>
                            ...
                            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 text-xs space-y-1">
                                ...
                                <div className="font-bold text-slate-900 dark:text-white">
                                    {viewingQuotation.company_name || '—'}
                                </div>
                                <div className="text-slate-500 text-[11px]">
                                    {viewingQuotation.company_address}
                                </div>
                                {viewingQuotation.company_phone && (
                                    <div className="text-slate-500 text-[11px]">
                                        Phone: {viewingQuotation.company_phone}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800 flex-wrap gap-2">
                            <div className="flex items-center gap-3">
                                <a
                                    href={`/quotations/${viewingQuotation.id}/print`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                ...
                                <a
                                    href={`/quotations/${viewingQuotation.id}/pdf`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                            ...
                            <div className="flex items-center gap-2">
                                {canEdit && viewingQuotation.status === 'accepted' && (
                                    <button
                                        type="button"
                                ...
                                <Link
                                    href={`/quotations/${viewingQuotation.id}`}
