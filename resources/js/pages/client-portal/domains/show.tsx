...
export default function ClientPortalDomainShow({
    ...
    const [confirmingPaidPayment, setConfirmingPaidPayment] = useState<DomainPaymentItem | null>(null);
    ...
    const handleMarkPaymentPaidSubmit = () => {
        ...
        router.post(
            `/client-portal/domains/payments/${confirmingPaidPayment.id}/mark-as-paid`,
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
    ...
    return (
        <ClientPortalLayout client={client} breadcrumbs={breadcrumbs}>
            ...
            <div className="p-2 sm:p-6 w-full space-y-6 bg-slate-50/50 dark:bg-slate-950">
                                                            ...
                                                            <div className="inline-flex items-center gap-1.5">
                                                                ...
                                                                {hasInvoice && !isPaid && hasPermission(user, 'edit-client-portal-domain-payments') && (
                                                                    <button
                                                                        ...
                                                                        title="Mark Domain Payment as Paid"
                                                                    >
                ...
                {/* MARK DOMAIN PAYMENT AS PAID CONFIRMATION MODAL */}
                {confirmingPaidPayment && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
                        <div className="w-full max-w-md max-h-[90vh] my-auto overflow-y-auto rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 shadow-2xl space-y-4 text-center animate-in fade-in zoom-in-95 duration-200 relative">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-base font-extrabold text-slate-900 dark:text-white">Mark Domain Payment as Paid</h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400">
                                    Confirm payment receipt for <strong>"{confirmingPaidPayment.title}"</strong> ({formatCurrency(confirmingPaidPayment.amount)}). This will renew the domain status and mark its linked invoice as Paid if all items are settled.
                                </p>
                            </div>
