...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Master Data', href: '/task-categories' },
...
interface TaskCategoriesIndexProps {
    categories: PaginatedData<TaskCategoryItem>;
    stats: {
        total: number;
        active: number;
        inactive: number;
...
export default function TaskCategoriesIndex({ categories, stats, filters }: TaskCategoriesIndexProps) {
    ...
    const [editingCategory, setEditingCategory] = useState<TaskCategoryItem | null>(null);
    const [deletingCategory, setDeletingCategory] = useState<TaskCategoryItem | null>(null);
    ...
    const handleCreateSubmit = (e: FormEvent) => {
        e.preventDefault();
        createForm.post('/task-categories', {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleEditOpen = (cat: TaskCategoryItem) => {
    ...
    const handleEditSubmit = (e: FormEvent) => {
        ...
        editForm.put(`/task-categories/${editingCategory.id}`, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleDeleteSubmit = () => {
        ...
        setIsDeleting(true);
        router.delete(`/task-categories/${deletingCategory.id}`, {
            preserveScroll: true,
            onFinish: () => {
                setIsDeleting(false);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Task Categories Management" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                            Define categories (Graphic Design, Server Maintenance, etc.) to classify general operational tasks.
                        </p>
                    </div>

                    {hasPermission(user, 'create-task-categories') && (
                        <button
                                        ...
                                        <tr key={cat.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    {hasPermission(user, 'edit-task-categories') && (
                                                        <button
                                                    ...
                                                    {hasPermission(user, 'delete-task-categories') && (
                                                        <button
                                ...
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400">
                                            Add new task classification type.
                                        </p>
                                    </div>
                                </div>
                                <button
                            ...
                            <form onSubmit={handleCreateSubmit} noValidate className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <Input
                                        ...
                                        placeholder="e.g. Graphic Design or Server Maintenance"
                                        required
                                        className={`h-11 rounded-xl bg-slate-50 dark:bg-slate-950 text-sm font-semibold text-slate-900 dark:text-white transition-all ${createForm.errors.name
                ...
                {editingCategory && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
                        <div className="w-full max-w-md rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
                            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                                <div className="flex items-center gap-2.5">
                                    <div className="p-2.5 rounded-xl bg-[#003796]/10 text-[#003796] dark:text-blue-400">
                            ...
                            <form onSubmit={handleEditSubmit} noValidate className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <Input
                                        ...
                                        required
                                        className={`h-11 rounded-xl bg-slate-50 dark:bg-slate-950 text-sm font-semibold text-slate-900 dark:text-white transition-all ${editForm.errors.name
                                            ? 'border-rose-500 ring-2 ring-rose-500/20 focus:border-rose-500 focus:ring-rose-500/20'
                                            : 'border-slate-200 dark:border-slate-800 focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10'
                                            }`}
                                    />
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
                        <div className="w-full max-w-md max-h-[90vh] my-auto overflow-y-auto rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 shadow-2xl space-y-4 text-center animate-in fade-in zoom-in-95 duration-200 relative">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-base font-black text-slate-900 dark:text-white">Delete Task Category?</h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400">
                                    Are you sure you want to delete <strong className="text-slate-900 dark:text-white">"{deletingCategory.name}"</strong>? This action cannot be undone.
                                </p>
                            </div>

                            {deletingCategory.tasks_count && deletingCategory.tasks_count > 0 ? (
                                <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-xs font-medium text-amber-800 dark:text-amber-300 text-left">
                                    <strong>Cannot Delete:</strong> This category is assigned to {deletingCategory.tasks_count} task(s). Reassign or delete those tasks first.
                                </div>
                            ) : null}
