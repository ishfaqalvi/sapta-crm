...
export default function UserCreate({ roles = [], clients = [], employees = [] }: UserCreateProps) {
    const [avatarPreview, setAvatarPreview] = useState<string | null>(null);
    const avatarInputRef = useRef<HTMLInputElement>(null);

    const clientOptions = (clients || []).map((c) => ({
        value: c.id,
        label: c.has_user ? `${c.name} (User Account Exists)` : c.name,
        subLabel: c.client_code ? `Code: ${c.client_code}` : undefined,
        disabled: Boolean(c.has_user),
    }));

    const employeeOptions = (employees || []).map((e) => ({
        value: e.id,
        label: e.has_user ? `${e.name} (User Account Exists)` : e.name,
        subLabel: e.employee_code ? `Code: ${e.employee_code}` : undefined,
        disabled: Boolean(e.has_user),
    }));
    ...
    const form = useForm({
        ...
        email: '',
        password: '',
        type: 'admin' as 'admin' | 'client' | 'employee',
        is_active: true as boolean,
        client_id: '' as string | number,
        employee_id: '' as string | number,
        roles: (roles.length > 0 ? [roles[0].name] : []) as string[],
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Create New System User" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                            Register system admin, staff employee login, or client portal account with custom roles.
                        </p>
                    </div>
                        ...
                        <div className="flex items-center gap-3 pb-4 border-b border-slate-100 dark:border-slate-800">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400">
                                    Full name, email address, secure password, and profile photo.
                                </p>
                            </div>
                        </div>
