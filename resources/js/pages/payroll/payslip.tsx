...
export default function Payslip({ payroll }: PayslipProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: `Payslip - ${payroll.employee.name}`,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Salary Slip - ${payroll.employee.name} (${monthName} ${payroll.year})`} />

            {/* Print Styles for Single Page Fit & Full Width */}
            <style>{`
                @media print {
                    @page {
                        size: A4 portrait;
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4 sm:p-6 bg-slate-50/50 dark:bg-slate-950 payslip-container">
                ...
                <div className="print:hidden flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                            Monthly PKR salary statement for <strong className="text-slate-900 dark:text-white">{payroll.employee.name}</strong> ({monthName} {payroll.year}).
                        </p>
                    </div>
                ...
                {/* Printable Payslip Card - Full Width Single Page */}
                <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-xl text-slate-900 payslip-card">
                    ...
                    {/* Employee Profile & Disbursement Details Grid */}
