...
interface ExpenseCategoriesIndexProps {
    categories: PaginatedData<ExpenseCategoryItem>;
    stats: {
        total: number;
        active: number;
        inactive: number;
...
export default function ExpenseCategoriesIndex({ categories, stats, filters }: ExpenseCategoriesIndexProps) {
    ...
    const handleEditOpen = (cat: ExpenseCategoryItem) => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Expense Categories Management" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                            Define categories (Office Rent, Server Hosting, Salaries, Utilities, etc.) to classify operational expenses.
                        </p>
                    </div>

                    {hasPermission(user, 'create-expense-categories') && (
                        <button
                            onClick={() => {
                                createForm.reset();
                                        ...
                                        <tr key={cat.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    {hasPermission(user, 'edit-expense-categories') && (
                                                        <button
                                                    ...
                                                    {hasPermission(user, 'delete-expense-categories') && (
                                                        <button
                                ...
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400">
                                            Add new expense classification type.
                                        </p>
                                    </div>
                                </div>
                                <button
                            ...
                            <form onSubmit={handleCreateSubmit} noValidate className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <Input
                                        ...
                                        placeholder="e.g. Office Rent, Server Hosting, Salaries, Utilities"
                                        required
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
                        <div className="w-full max-w-md max-h-[90vh] my-auto overflow-y-auto rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 shadow-2xl space-y-4 text-center animate-in fade-in zoom-in-95 duration-200 relative">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-base font-black text-slate-900 dark:text-white">Delete Expense Category?</h3>
                            ...
                            {deletingCategory.expenses_count && deletingCategory.expenses_count > 0 ? (
                                <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-xs font-medium text-amber-800 dark:text-amber-300 text-left">
                                    <strong>Cannot Delete:</strong> This category is assigned to {deletingCategory.expenses_count} expense entry(ies). Reassign or delete those entries first.
                                </div>
                            ) : null}
