...
class TaskUpdatedMail extends Mailable
{
     ...
     * @param array $changes
     */
    public function __construct($task, string $taskType, Employee $employee, ?User $updatedBy = null, array $changes = [])
    {
        $this->task = $task;
    ...
    protected function buildDetails(): array
    {
        ...
        $sourceTitle = 'General Operations';
        $clientName = null;
        ...
        $actionUrl = url("/my-tasks/task/{$this->taskType}/{$this->task->id}/conversation");
        ...
        return [
            ...
            'updated_by_name' => $this->updatedBy ? $this->updatedBy->name : 'Administrator',
    ...
    public function envelope(): Envelope
    {
        $subject = match ($this->taskType) {
            'project' => "Task Updated: {$this->details['task_title']} ({$this->details['source_title']})",
            'service' => "Task Updated: {$this->details['task_title']} ({$this->details['source_title']})",
            default => "Task Updated: {$this->details['task_title']} ({$this->details['task_code']})",
        };
