...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Income Categories', href: '/income-categories' },
];
...
interface IncomeCategoriesIndexProps {
    categories: PaginatedData<IncomeCategoryItem>;
    stats: {
        total: number;
        active: number;
        inactive: number;
...
export default function IncomeCategoriesIndex({ categories, stats, filters }: IncomeCategoriesIndexProps) {
    ...
    const [editingCategory, setEditingCategory] = useState<IncomeCategoryItem | null>(null);
    const [deletingCategory, setDeletingCategory] = useState<IncomeCategoryItem | null>(null);
    ...
    const handleCreateSubmit = (e: FormEvent) => {
        e.preventDefault();
        createForm.post('/income-categories', {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleEditOpen = (cat: IncomeCategoryItem) => {
    ...
    const handleEditSubmit = (e: FormEvent) => {
        ...
        editForm.put(`/income-categories/${editingCategory.id}`, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleDeleteSubmit = () => {
        ...
        setIsDeleting(true);
        router.delete(`/income-categories/${deletingCategory.id}`, {
            preserveScroll: true,
            onFinish: () => {
                setIsDeleting(false);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Income Categories Management" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                            Define categories (Client Project Payment, Retainer Fee, Software License, etc.) to classify business income.
                        </p>
                    </div>

                    {hasPermission(user, 'create-income-categories') && (
                        <button
                            onClick={() => {
                                createForm.reset();
                        ...
                        <table className="w-full text-left text-xs text-slate-600 dark:text-slate-300">
                            <thead className="bg-slate-50 dark:bg-slate-950/70 border-b border-slate-200/80 dark:border-slate-800 uppercase tracking-wider text-[11px] font-bold text-slate-500 dark:text-slate-400">
                                <tr>
                                    ...
                                    <th className="px-6 py-4">Assigned Entries</th>
                                    ...
                                    categories.data.map((cat) => (
                                        <tr key={cat.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                ...
                                                <span className="text-slate-400 text-xs ml-1">entries</span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    {hasPermission(user, 'edit-income-categories') && (
                                                        <button
                                                    ...
                                                    {hasPermission(user, 'delete-income-categories') && (
                                                        <button
                                ...
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400">
                                            Add new income classification type.
                                        </p>
                                    </div>
                                </div>
                                <button
                            ...
                            <form onSubmit={handleCreateSubmit} noValidate className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <Input
                                        ...
                                        placeholder="e.g. Client Project Payment, Retainer Fee, Software License"
                                        required
                                ...
                                <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                                    ...
                                    <Button
                                        ...
                                        className="h-10 px-5 rounded-xl bg-gradient-to-r from-[#003796] via-[#0052D4] to-[#1d4ed8] hover:opacity-95 text-white text-xs font-bold shadow-md shadow-blue-600/20 active:scale-[0.99] transition-all cursor-pointer inline-flex items-center gap-2"
                                    >
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
                        <div className="w-full max-w-md max-h-[90vh] my-auto overflow-y-auto rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 shadow-2xl space-y-4 text-center animate-in fade-in zoom-in-95 duration-200 relative">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-base font-black text-slate-900 dark:text-white">Delete Income Category?</h3>
                            ...
                            {deletingCategory.incomes_count && deletingCategory.incomes_count > 0 ? (
                                <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-xs font-medium text-amber-800 dark:text-amber-300 text-left">
                                    <strong>Cannot Delete:</strong> This category is assigned to {deletingCategory.incomes_count} income entry(ies). Reassign or delete those entries first.
                                </div>
                            ) : null}
